...
#' @export
#' @import lubridate sp tidyverse ecmwfr ncdf4 udunits2 sf rgdal raster rgeos stringr zoo lwgeom tibble abind foreach doParallel
#' @examples
...
get_fire_weather <- function(study.area, my.fires,output.folder,utc.zone,wf_user,wf_key) {
  ...
  for (i in 1:nrow(my_fires_t_dated_2020)){
    ...
    write.csv(result_hours_final_df,"fire_weather_study_area_.csv",row.names = FALSE)
