...
#' @param my.fires Polygon shapefile containing the fire perimeters (dated or not). This shapefile will be used to characterize the fire regime and to propose the durations to use in the MTT simulation. Must contain a field “Year”.
...
#' @param min.size Numerical. Minimum fire size to be considered in the analysis. Useful to subset the fire sample
...
build_report <- function(study.area, my.fires,my.dated.fires,meteo.data,active.period,user.period,meteo.aggregation,
  ...
  if (create.clusters==TRUE){
    ...
    my_doc <- my_doc %>%
      ...
      body_add_par(paste("The figure below illustrates the contribution of burned area in each fire size class to the overall burned area. The fire size classes are in hectares"),style = "Normal") %>%
      body_add_par("")%>%
