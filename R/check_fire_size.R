...
evaluate_fire_size <- function (Folder.Outputs,
  ...
  unique_all <- data.frame(n_unique)
  
  unique_all <- unique_all %>% separate(n_unique, c("durval", "durclass"), sep="_durclass")
  unique_all$durclass <- paste("durclass",unique_all$durclass,sep="")
  ...
  results_inter_expanded<-data.frame(results_inter_expanded)
  
  
  results_i_final <- data.frame(matrix(nrow = 0, ncol = 2 * ncol(results_inter_expanded)))
  
  results_i_final_loop <- data.frame(matrix(nrow = 16, ncol = 2 * ncol(results_inter_expanded)))
  
  #results_i_final_loop$combo <- NA
  #results_i_final_loop$sum_use <- NA
  
  if (all.dist==TRUE){
    
    for (j in 1:nrow(results_inter_expanded)){
      
      for (k in 1:ncol(results_inter_expanded)){
        
        name <- as.character(results_inter_expanded[j,k])
        name <- paste(name,"_durclass_",k,sep="")
        
        durval_name <- sub("_durclass_.*", "", name)
        durclass_name <- substrRight(name, 10)
        
        results_use <- subset(results,size>=0)
        results_i <- subset(results_use, combo==name)
        
        
        if (is.na(intervals[1])) {
          int_1 <- NA } else {
            int_1 <- sum(results_i$size < intervals[1])
          }
        
        
        if (is.na(intervals[2])) {
          int_2 <- NA } else {
            int_2 <- sum(results_i$size >= intervals[1] & results_i$size < intervals[2])
          }
        
        
        if (is.na(intervals[3])) {
          int_3 <- NA } else {
            int_3 <- sum(results_i$size >= intervals[2] & results_i$size < intervals[3])
          }
        
        if (is.na(intervals[4])) {
          int_4 <- NA } else {
            int_4 <-sum(results_i$size >= intervals[3] & results_i$size < intervals[4])
          }
        
        
        if (is.na(intervals[5])) {
          int_5 <- NA } else {
            int_5 <-sum(results_i$size >= intervals[4] & results_i$size < intervals[5])
          }
        
        
        if (is.na(intervals[6])) {
          int_6 <- NA } else {
            int_6 <-sum(results_i$size >= intervals[5] & results_i$size < intervals[6])
          }
        
        
        if (is.na(intervals[7])) {
          int_7 <- NA } else {
            int_7 <-sum(results_i$size >= intervals[6] & results_i$size < intervals[7])
          }
        
        
        if (is.na(intervals[8])) {
          int_8 <- NA } else {
            int_8 <-sum(results_i$size >= intervals[7] & results_i$size < intervals[8])
          }
        ...
        if (is.na(intervals[9])) {
          int_9 <- NA } else {
            int_9 <-sum(results_i$size >= intervals[8] & results_i$size < intervals[9])
          }
        ...
        if (is.na(intervals[10])) {
          int_10 <- NA } else {
            int_10 <-sum(results_i$size >= intervals[9] & results_i$size < intervals[10])
          }
        ...
        if (is.na(intervals[11])) {
          int_11 <- NA } else {
            int_11 <-sum(results_i$size >= intervals[10] & results_i$size < intervals[11])
          }
        ...
        if (is.na(intervals[12])) {
          int_12 <- NA } else {
            int_12 <-sum(results_i$size >= intervals[11] & results_i$size < intervals[12])
          }
        ...
        if (is.na(intervals[13])) {
          int_13 <- NA } else {
            int_13 <-sum(results_i$size >= intervals[12] & results_i$size < intervals[13])
          }
        
        
        if (is.na(intervals[14])) {
          int_14 <- NA } else {
            int_14 <-sum(results_i$size >= intervals[13] & results_i$size < intervals[14])
          }
        
        
        if (is.na(intervals[15])) {
          int_15 <- NA } else {
            int_15 <-sum(results_i$size >= intervals[14] & results_i$size < intervals[15])
          }
        
        if (is.na(length(intervals))) {
          int_last <- NA } else {
            int_last <-sum(results_i$size >= intervals[length(intervals)])
          }
        
        
        
        results_i_final_loop[,k] <- as.data.frame(rbind(int_1,int_2,int_3,int_4,int_5,
        ...
        results_i_final_loop$combo <- j
        results_i_final_loop$sum_use <- rowSums(results_i_final_loop[1:k],na.rm = T)
        
        results_i_final_loop[,ncol(results_i_final_loop)-1-k] <- name
        names(results_i_final_loop)[ncol(results_i_final_loop)-1-k] <- paste0('durclass_',k)
        results_i_final_loop[,ncol(results_i_final_loop)-1-k]<-gsub(paste0("_durclass_",k),"",results_i_final_loop[,ncol(results_i_final_loop)-1-k])
      }
      
      
      results_i_final <- rbind(results_i_final,results_i_final_loop)
    ...
    results_i_final$x_axis <- rep(1:nrow(subset(results_i_final,combo==1)))
  }
  ...
  if (all.dist==FALSE){
    ...
    results_i_final_loop <- data.frame(matrix(nrow = 16-1, ncol = 2 * ncol(results_inter_expanded)))
    ...
    for (j in 1:nrow(results_inter_expanded)){
      ...
      for (k in 1:ncol(results_inter_expanded)){
        ...
        results_i_final_loop[,k] <- as.data.frame(rbind(int_2,int_3,int_4,int_5,
                                                        int_6,int_7,int_8,int_9,int_10,
                                                        int_11,int_12,int_13,int_14,int_15,
                                                        int_last))
  ...
  results_inter<-results_i_final
  
  unique_all<-nrow(results_inter)
  
  #
  # ncol(results_inter_expanded)
  #
  # results_inter
  #
  #
  ...
  if (is.na(intervals[1])) {
    int_1 <- NA } else {
      int_1 <- sum(my_observed_fire$Fire.size < intervals[1])
    }
  
  
  if (is.na(intervals[2])) {
    int_2 <- NA } else {
      int_2 <- sum(my_observed_fire$Fire.size >= intervals[1] & my_observed_fire$Fire.size < intervals[2])
    }
  
  
  if (is.na(intervals[3])) {
    int_3 <- NA } else {
      int_3 <- sum(my_observed_fire$Fire.size >= intervals[2] & my_observed_fire$Fire.size < intervals[3])
    }
  
  if (is.na(intervals[4])) {
    int_4 <- NA } else {
      int_4 <-sum(my_observed_fire$Fire.size >= intervals[3] & my_observed_fire$Fire.size < intervals[4])
    }
  
  
  if (is.na(intervals[5])) {
    int_5 <- NA } else {
      int_5 <-sum(my_observed_fire$Fire.size >= intervals[4] & my_observed_fire$Fire.size < intervals[5])
    }
  
  
  if (is.na(intervals[6])) {
    int_6 <- NA } else {
      int_6 <-sum(my_observed_fire$Fire.size >= intervals[5] & my_observed_fire$Fire.size < intervals[6])
    }
  
  
  if (is.na(intervals[7])) {
    int_7 <- NA } else {
      int_7 <-sum(my_observed_fire$Fire.size >= intervals[6] & my_observed_fire$Fire.size < intervals[7])
    }
  
  
  if (is.na(intervals[8])) {
    int_8 <- NA } else {
      int_8 <-sum(my_observed_fire$Fire.size >= intervals[7] & my_observed_fire$Fire.size < intervals[8])
    }
  ...
  if (is.na(intervals[9])) {
    int_9 <- NA } else {
      int_9 <-sum(my_observed_fire$Fire.size >= intervals[8] & my_observed_fire$Fire.size < intervals[9])
    }
  ...
  if (is.na(intervals[10])) {
    int_10 <- NA } else {
      int_10 <-sum(my_observed_fire$Fire.size >= intervals[9] & my_observed_fire$Fire.size < intervals[10])
    }
  ...
  if (is.na(intervals[11])) {
    int_11 <- NA } else {
      int_11 <-sum(my_observed_fire$Fire.size >= intervals[10] & my_observed_fire$Fire.size < intervals[11])
    }
  ...
  if (is.na(intervals[12])) {
    int_12 <- NA } else {
      int_12 <-sum(my_observed_fire$Fire.size >= intervals[11] & my_observed_fire$Fire.size < intervals[12])
    }
  ...
  if (is.na(intervals[13])) {
    int_13 <- NA } else {
      int_13 <-sum(my_observed_fire$Fire.size >= intervals[12] & my_observed_fire$Fire.size < intervals[13])
    }
  
  
  if (is.na(intervals[14])) {
    int_14 <- NA } else {
      int_14 <-sum(my_observed_fire$Fire.size >= intervals[13] & my_observed_fire$Fire.size < intervals[14])
    }
  
  
  if (is.na(intervals[15])) {
    int_15 <- NA } else {
      int_15 <-sum(my_observed_fire$Fire.size >= intervals[14] & my_observed_fire$Fire.size < intervals[15])
    }
  
  
  if (is.na(length(intervals))) {
    int_last <- NA } else {
      int_last <-sum(my_observed_fire$Fire.size >= intervals[length(intervals)])
    }
  ...
  my_observed_fire_final$RF <- my_observed_fire_final$V1/sum(my_observed_fire_final$V1)
  ...
  for (t in 1:length(unique(df$combo))){
    ...
    all_dur_loop <- subset(df,combo==t)
    ...
    all_dur_loop$durval_1 <- if(length(all_dur_loop$durclass_1)>0){
      gsub("durval_","",all_dur_loop$durclass_1)}
    
    all_dur_loop$durval_2 <- if(length(all_dur_loop$durclass_2)>0){
      gsub("durval_","",all_dur_loop$durclass_2)}
    
    all_dur_loop$durval_3 <- if(length(all_dur_loop$durclass_3)>0){
      gsub("durval_","",all_dur_loop$durclass_3)}
    
    all_dur_loop$durval_4 <- if(length(all_dur_loop$durclass_4)>0){
      gsub("durval_","",all_dur_loop$durclass_4)}
    
    all_dur_loop$durval_5 <- if(length(all_dur_loop$durclass_5)>0){
      gsub("durval_","",all_dur_loop$durclass_5)}
    ...
    test_obs$V1_sim <- test_i$FR
    test_obs$V1 <- test_obs$RF
    ...
    results[start_results:end_results,1]<- all_dur_loop_bind$FR
    ...
    results_rmse[start_results_rmse:end_results_rmse,2]<- as.numeric(gsub("durval_","",all_dur_loop$durclass_1[1]))
    results_rmse[start_results_rmse:end_results_rmse,3]<- if(length(all_dur_loop$durclass_2)>0){
      as.numeric(gsub("durval_","",all_dur_loop$durclass_2 [1]))}else{NA}
    
    results_rmse[start_results_rmse:end_results_rmse,4]<- if(length(all_dur_loop$durclass_3)>0){
      as.numeric(gsub("durval_","",all_dur_loop$durclass_3 [1]))}else{NA}
    
    results_rmse[start_results_rmse:end_results_rmse,5]<- if(length(all_dur_loop$durclass_4)>0){
      as.numeric(gsub("durval_","",all_dur_loop$durclass_4 [1]))}else{NA}
    
    results_rmse[start_results_rmse:end_results_rmse,6]<- if(length(all_dur_loop$durclass_5)>0){
      as.numeric(gsub("durval_","",all_dur_loop$durclass_5 [1]))}else{NA}
  ...
  results_rmse <- results_rmse[,colSums(is.na(results_rmse))<nrow(results_rmse)]
  ...
  my_observed_fire_final<-my_observed_fire_final[,-1]
  my_observed_fire_final$V1<-my_observed_fire_final$RF
  my_observed_fire_final<-my_observed_fire_final[,-3]
